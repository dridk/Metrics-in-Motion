...
vg.data.facet = function() {
  ...
  return stats;
};vg.data.stringtransform = function () {
    var fields = [],
        as = "label",
        separator = " ",
        substring = {
    ...
    var stringtransform = vg.data.mapper(function (d) {
        var l = fields.length,
            text = "",
            values = [],
            _substring = {
                substr: substring.len != "",
                slice: substring.end != "",
                start: +substring.start,
                len: +substring.len,
                end: +substring.end
            },
            _replace = replace.search != "",
            _truncate = truncate.limit != "";

        for (var i = 0; i < l; i++) {
            values.push(vg.accessor(fields[i])(d));
        }
        text = "" + values.join(separator);
        text = _substring.substr ?
            text.substr(_substring.start, _substring.len) : text;
        text = _substring.slice ?
            text.slice(_substring.start, _substring.end) : text;
        text = _replace ?
            text.replace(replace.search, replace.replace) : text;
        text = toLowerCase ?
            text.toLowerCase() : text;
        text = toUpperCase ?
            text.toUpperCase() : text;
        text = _truncate ?
            vg.truncate(text, truncate.limit, truncate.position, truncate.wordBreak, truncate.ellipsis):text;
        
        return (d[as] = text, d);
    });

    stringtransform.fields = function (fieldList) {
        fields = vg.array(fieldList);
        return stringtransform;
    };

    stringtransform.as = function (field) {
        as = field;
        return stringtransform;
    };

    stringtransform.separator = function (str) {
        separator = str + "";
        return stringtransform;
    };

    stringtransform.substring = function (map) {
        vg.keys(substring).forEach(function (k) {
            if (map[k] !== undefined) {
                substring[k] = map[k];
            }
        });
    ...
    stringtransform.replace = function (map) {
        vg.keys(replace).forEach(function (k) {
            if (map[k] !== undefined) {
                replace[k] = map[k];
            }
        });
    ...
    stringtransform.truncate = function (map) {
        vg.keys(truncate).forEach(function (k) {
            if (map[k] !== undefined) {
                truncate[k] = map[k];
            }
        });
    ...
    stringtransform.toLowerCase = function (b) {
        toLowerCase = !! b;
        return stringtransform;
    };

    stringtransform.toUpperCase = function (b) {
        toUpperCase = !! b;
        return stringtransform;
    };
    return stringtransform;
